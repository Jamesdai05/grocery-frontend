...
export const updateCart=(state)=>{
    ...
    state.itemPrice=numberFormating(state.cartItems.reduce((acc,item)=>acc+item.price * item.qty,0))

    state.itemShippingCost=numberFormating(state.itemsPrice>200 ? 0 :  10 );

    state.taxPrice=numberFormating(Number(state.itemsPrice) * 0.1)

    state.totalPrice=numberFormating(Number(state.itemPrice) + Number(state.itemShippingCost) + Number(state.taxPrice))
