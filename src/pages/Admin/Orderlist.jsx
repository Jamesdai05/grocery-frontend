...
const OrderList = () => {
    ...
    const { data: allOrders, isLoading, error, refetch } = useFetchAllOrders();

    const handleClearAndRefetch=async()=>{
        queryClient.removeQueries(["allOrders"]); // clears cache
        await queryClient.prefetchQuery({
            queryKey: ["allOrders"],
            queryFn: fetchAllOrders, // refetch backend data immediately
        });
    }
    ...
    return (
        <div className="admin-list-container">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={handleClearAndRefetch} className="flex gap-2 items-center bg-blue-200 p-1 rounded text-blue-800 hover:text-white hover:bg-blue-500 duration-300
                ">
                    <LuRefreshCw /> Refresh
                </button>
            </div>
